...
describe('Create Todo Item', function () {
  ...
  it('should receive a location hyperlink', function () {
    return assert(result, 'header.location').to.match(
      /^https?:\/\/.+\/todos\/[\d]+$/
    );
  });

  it('should create the item', function () {
    let item = result.then(function (res) {
...
describe('Delete Todo Item', function () {
  ...
  it('should delete the item', function () {
    let result = del(location).then(function (res) {
      return get(location);
    });
    return assert(result).to.eventually.be.rejectedWith('Not Found');
  });
});
